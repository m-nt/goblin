...
class Match_controller {
    ...
    async user_exists(uuid) {
        try {
            let exists = await pubClient.exists(`${this._users_key_prefix}${uuid}`)
            return exists == 1
        } catch (error) {
            return false
        }
    ...
    async match_exists(muid) {
        try {
            let exists = await pubClient.exists(`${this._matches_key_prefix}${muid}`)
            return exists == 1
        } catch (error) {
            return false
        }
    ...
    async get_all_users() {
        ...
        try {
            ...
            LOGGER("Match Controller get_all_users ->", "client", reason, 500, 10)
            return []
        }
    ...
    async get_all_matches() {
        ...
        try {
            ...
            LOGGER("Match Controller get_all_matches ->", "client", reason, 500, 10)
            return []
        }
        for (let i = 0; i < keys.length; i++) {
            const item = keys[i];
    ...
    async add_user(_user, overwrite=false) {
        ...
        pubClient.set(`${this._users_key_prefix}${user.uuid}`,JSON.stringify(user))
    }
    /**
     * @param {string} uuid 
     * @returns {Promise<boolean>}
    ...
    async remove_user(uuid) {
        try {
        let res = await pubClient.expire(`${this._users_key_prefix}${uuid}`,1)
        return res
        } catch (error) {
            return false
        }
    ...
    async add_match(_match, overwrite=false) {
        ...
        if (error) return LOGGER("server", "client", error, 500, 10)
        let exists = await this.match_exists(match.uuid)
        if (exists && !overwrite) return
        pubClient.set(`${this._matches_key_prefix}${match.uuid}`,JSON.stringify(match))
    }
    /**
     * @param {string} muid 
     * @returns {Promise<boolean>}
    ...
    async remove_match(muid) {
        try {
        let res = await pubClient.expire(`${this._matches_key_prefix}${muid}`,1)
        return res
        } catch (error) {
            return false
        }
    ...
    async loadMatchAsync(muid) {
        ...
        let { match, error } = User.from_json(_match)
